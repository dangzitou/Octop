...
### 修复
- 同一 Agent 的连接器凭据刷新、临时用户上下文与重载共用互斥锁，保留不同 Agent 的并行重载（Refs #799）。
...
- 审计日志接口 `GET /api/admin/audit-log` 的 `limit` 增加 1–500 边界（与设置 → 安全页可选的最大条数一致）：此前负数会被 SQLite 解释成「不限制」，一次返回全部审计记录，`limit=0` 又返回空列表；越界请求现在统一拒绝
